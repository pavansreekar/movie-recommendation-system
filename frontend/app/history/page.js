...
export default function HistoryPage() {
  ...
  async function loadHistory() {
    ...
    setItems(payload.watched_items || []);
  }

  useEffect(() => {
    if (session.authenticated) loadHistory();
  ...
  return (
    <AppShell user={session.user}>
      <section className="hero premium-card compact-hero" style={{ padding: "1.5rem 1.8rem" }}>
        <div className="hero-with-filters">
          <div className="hero-text">
            ...
            <p style={{ margin: 0 }}>
              Your watched history lives here.
              {items.length > 0 && (
      ...
      {items.length === 0 ? (
        <div className="empty-state premium-card">No watched history yet.</div>
      ) : filtered.length === 0 ? (
